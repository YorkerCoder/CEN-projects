...
using Plots; gr()
#there's a spike in temperature as expected
#more of a gets converted so more b is used and more c is made
#but after it rebounds back to steady state concentrations
...
function p32()
    ...
    parameters = [ΔH, cpa, cpb, cpc, cpm, va, vb, vm, Vr, T0,
...
function p32!(du, u, p, t)
    ca, cb, cc, T = u
    ΔH, cpa, cpb, cpc, cpm, va, vb, vm, Vr, T0, faf, fbf, fmf, Ta, R, Qf, tau, caf, cbf, ccf, cmf = p
    ...
    #change ua by half after 10 hours
    if t > 10.0 && t<10.5
        ua = ua*0.5
    end
